      ...
      if (req.user) {
        User.findOne({ google: profile.id }, function(err, user) {
          if (user) {
            req.flash('error', {
              msg:
                'There is already an existing account linked with Google that belongs to you.'
            });
          } else {
            User.findById(req.user.id, function(err, user) {
      ...
      if (req.user) {
        User.findOne({ twitter: profile.id }, function(err, user) {
          if (user) {
            req.flash('error', {
              msg:
                'There is already an existing account linked with Twitter that belongs to you.'
            });
            done(err);
          } else {
            User.findById(req.user.id, function(err, user) {
              ...
              user.save(function(err) {
                req.flash('success', {
                  msg: 'Your Twitter account has been linked.'
                });
                done(err, user);
              });
            });
        ...
        User.findOne({ github: profile.id }, function(err, user) {
          if (user) {
            ...
            User.findById(req.user.id, function(err, user) {
              ...
              user.save(function(err) {
                req.flash('success', {
                  msg: 'Your Github account has been linked.'
                });
                done(err, user);
              });
            });
